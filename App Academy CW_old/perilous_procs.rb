...
def exactly?(array,n,&block)
    ...
    array.each do |walter|
        if block.call(walter)
            count += 1
        end
    end
count == n
...
def filter_out(array, &block)
    filtered = []
    array.each do |ele|
        filtered << ele if !block.call(ele)
...
def every?(array, &block)
    truearray = []
    array.each do |ele|
        truearray << block.call(ele)
    end
    truearray.uniq.length == 1 && truearray[0] == true
end
...
def proctition_platinum(array, *procs)
    ...
    while i <= procs.length
        ...
        new_hash[i] = key_arr(array, procs[i-1])

        i+=1 
    end
    new_hash.each do |k,v|
        v.select! {|ele| !used.include?(ele)}
        v.each {|ele| used << ele} 
    end
...
#     procs.each do |proc|
#         if proc.call(array[i])
#                 new_hash[i+1] += [array[i]]
#         end
#     end
#     i += 1
# end
...
def key_arr(array, prc) # "when!", "WHERE!"]
    key_arr = []
...
# # p key_arr(['WHO', 'what', 'when!', 'WHERE!', 'how', 'WHY'], contains_a ) #["when!", "WHERE!"]
...
def picky_procipher (sentence, proc_hash)
    ...
    return_array = old_array.map do |word|
        temp = word
